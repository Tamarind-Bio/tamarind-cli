...
The tool catalog and per-tool schemas are gated by per-org visibility logic that runs
server-side, so the client consumes them over HTTP from the catalog service (the
``/catalog/*`` routes) rather than reading the database directly.

These routes return exactly the JSON the MCP's discovery tools return
(``getAvailableTools``, ``listModalities``, ``listTags``, ``getJobSchema``), because
both are served by the same shared implementation. So whatever a tool looks like in
the MCP, it looks identical here.
"""
